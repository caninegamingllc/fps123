...
public class NeoStackAI : ModuleRules
{
	public NeoStackAI(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (Directory.Exists(LuaIncludePath))
		{
		...
		if (Directory.Exists(Sol2IncludePath))
		{
		...
		PrivateDependencyModuleNames.AddRange(new string[]
		{
			...
			// and the socket to the detached agent runtime daemon
			"Sockets",
			// Editor close guard for the detached runtime (RegisterCanCloseEditor)
			"MainFrame",
		});
		...
		// Native Claude Desktop MCPB creation and bounded in-memory readback.
		PrivateDependencyModuleNames.Add("FileUtilities");
	...
	private IEnumerable<string> GetLikelyPluginDescriptorPaths(ReadOnlyTargetRules Target, string PluginName)
	{
		...
		var KnownPaths = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
		{
			...
			{ "ModelContextProtocol", Path.Combine("Experimental", "ModelContextProtocol", "ModelContextProtocol.uplugin") },
			{ "DynamicWind",       Path.Combine("Experimental", "DynamicWind", "DynamicWind.uplugin") },
			{ "Composure",         Path.Combine("Compositing", "Composure", "Composure.uplugin") },
			{ "MediaIOFramework",  Path.Combine("Media", "MediaIOFramework", "MediaIOFramework.uplugin") },
			{ "GameplayCameras",   Path.Combine("Cameras", "GameplayCameras", "GameplayCameras.uplugin") },

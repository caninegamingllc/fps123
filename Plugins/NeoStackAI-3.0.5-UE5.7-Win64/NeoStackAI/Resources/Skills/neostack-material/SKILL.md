...
The first `add_node` on an unopened Material or Material Function may yield
once while the editor creates that preview. This is an intentional transaction
checkpoint: editor-owned rooted preview objects are initialized before any user
graph mutation, so a later Lua failure can roll back the graph safely. Treat the
returned node normally; do not open or warm the editor in a separate workaround.
...
| The first `add_node` takes an extra editor tick | Expected for an unopened Material; preview initialization is checkpointed before graph mutation. |

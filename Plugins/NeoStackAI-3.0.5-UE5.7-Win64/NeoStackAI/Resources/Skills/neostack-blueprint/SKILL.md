...
For pools or any Blueprint that moves components independently at runtime, create a
stable `SceneComponent` root first and pass it as the parent of every movable child:

```lua
bp:add_component("SceneRoot", "SceneComponent")
bp:add_component("PoolSlot0", "DecalComponent", "SceneRoot")
bp:add_component("PoolSlot1", "DecalComponent", "SceneRoot")
```

Never make a pooled slot the actor root: setting that component's world transform
also moves the owning actor and therefore every sibling slot. `reparent_component`
is safe to call when the requested parent is already current; the SCS component
sequence and `info().num_components` stay unique. Calling it once on an asset
created by an older affected build also normalizes duplicate cached entries.
...
Action discovery is deliberately read-only. Opening an unloaded Blueprint may
yield once while package `PostLoad` finishes outside the user transaction, and
an ambiguous or missing `add_node` result does not dirty or compile the graph.
If you just authored a custom function/event that must appear in the action
database, compile and save it first, then discover and add the call node in a
fresh script. Do not depend on discovery to perform a hidden compile.

...
6. Configure and wire any locomotion BlendSpace from measured runtime inputs.
7. Wire transition rules with caller-chosen thresholds.
8. Compile, save, and verify from a fresh call.
...
Use `AnimInstance` as the parent class for a normal character AnimBP.
...
## Build a directional locomotion BlendSpace

Treat a directional BlendSpace as a sampled continuous surface, not a list of
clips that merely looks complete in the editor. Discover compatible animations
for the exact skeleton, then configure axes and smoothing explicitly:

```lua
local blendspace = open_asset("/Game/Characters/BS_Locomotion")
assert(blendspace)
assert(blendspace:configure("axis", {
  axis = 0, display_name = "Direction", min = -180, max = 180,
  grid_divisions = 8, snap_to_grid = true,
}))
assert(blendspace:configure("axis", {
  axis = 1, display_name = "Speed", min = 0, max = run_speed,
  grid_divisions = speed_divisions, snap_to_grid = true,
}))
assert(blendspace:configure("interpolation", {
  axis = 0, type = "SpringDamper", time = direction_smoothing,
  damping_ratio = direction_damping, max_speed = direction_rate,
}))
assert(blendspace:configure("interpolation", {
  axis = 1, type = "SpringDamper", time = speed_smoothing,
  damping_ratio = speed_damping, max_speed = acceleration_rate,
}))
assert(blendspace:configure("settings", {
  loop = true,
  allow_marker_sync = true,
  interpolate_using_grid = false,
  triangulation_direction = "Tangential",
  weight_ease_in_out = true,
}))
```

Derive the values from the character movement model and source clips; the
identifiers above are placeholders, not defaults. For full eight-way movement:

- populate idle, walk, and run speed rings rather than only the maximum-speed
  perimeter;
- cover forward, both forward diagonals, both lateral directions, both back
  diagonals, and back;
- close the `-180`/`+180` seam deliberately, either with compatible duplicate
  back samples or a separately verified wrapping policy;
- use compatible sync markers before enabling marker-based sync;
- keep character facing fixed during directional proof, or calculate direction
  in actor-local space. World-space `atan2` is valid only when facing is
  intentionally fixed.

Wire the BlendSpace Player's asset through the stored `BlendSpace` pin and read
it back; do not accept a node title as proof that the task asset is assigned:

```lua
local state = assert(bp.graphs[walk_run_graph])
local changed, details = state:set_pin(player_handle, "BlendSpace", blendspace_path)
assert(changed and details and details.target == "anim_asset_player")
assert(string.find(state:get_pin(player_handle, "BlendSpace"), blendspace_path, 1, true))
```

Before PIE, probe representative coordinates with
`blendspace:evaluate({direction, speed}, {combine_animations=true})`. Require a
non-empty result, expected dominant clips at the cardinal/diagonal points, and
weights appropriate for an intermediate gait. Repeat those probes after a cold
restart.

Runtime proof must use the same live direction and speed that feed the AnimBP.
Drive idle, an intermediate walk, maximum-speed cardinal/diagonal states, and a
return to idle. Poll `playtest_read_state` until velocity and direction reach
the requested state; a fixed delay can sample before the next movement tick.
Keep decorative benchmark geometry outside the character's path or disable its
collision so a prop cannot masquerade as an animation or movement failure.
Capture every state with one stable camera, open every original frame, and
reject reference-pose flashes, mesh snaps, discontinuous facing, or a gait that
does not match the measured velocity.

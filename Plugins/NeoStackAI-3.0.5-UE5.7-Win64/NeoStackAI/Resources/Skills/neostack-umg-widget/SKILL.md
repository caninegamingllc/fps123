...
Author float-property tracks with ordered, in-range keys. The property is
resolved from the target widget class, so this is not limited to
`RenderOpacity` or a particular widget type:
...
local track = bp:set_animation_float_track("IntroPulse", {
  widget="EngageBtn",
...
if not track or track.key_count ~= 3 then
  error("Opacity track was not persisted")
end

local compile = bp:compile()
if not compile or not compile.success or compile.error_count ~= 0 then
  error("Widget compile failed")
end
bp:save()
```

Calling `set_animation_float_track` again for the same widget and property
replaces its keys without creating a duplicate track. Times must be strictly
increasing, resolve to unique frames, and remain inside the animation range.
Valid interpolation modes are `linear`, `constant`, and `cubic`; a key can
override the track default.

Verify the binding, property path, and keys after reopening:

```lua
local bp = open_asset("/Game/UI/WBP_FieldRelay")
local animation = bp:get_animation("IntroPulse")
if not animation or #animation.tracks ~= 1 then
  error("Expected one stored animation track")
end

local track = animation.tracks[1]
if track.widget_name ~= "EngageBtn"
    or track.property_path ~= "RenderOpacity" then
  error("Animation binding or property mismatch")
end

local keys = track.sections[1].keys
if #keys ~= 3
    or math.abs(keys[1].time - 0.0) > 0.001
    or math.abs(keys[1].value - 0.35) > 0.001
    or math.abs(keys[2].time - 0.6) > 0.001
    or math.abs(keys[2].value - 1.0) > 0.001
    or math.abs(keys[3].time - 1.2) > 0.001
    or math.abs(keys[3].value - 0.35) > 0.001 then
  error("Stored animation keys do not match")
end
```
...
| Animation track creation returns `nil` | Confirm the widget and reflected float property names, use ordered unique-frame key times, and keep every key inside the configured animation range. |
| Animation exists but `track_count=0` | `add_animation` creates timing only. Call `set_animation_float_track`, save, reopen, then inspect `get_animation(name).tracks[].sections[].keys`. |
...
- `bp:set_animation_float_track(name, opts)` — generalized widget
  float-property binding and key authoring.

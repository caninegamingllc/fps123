...
local ok = ns:configure("module", "InitializeParticle", {
  ...
  stage = "ParticleSpawn",
  parameters = {
    -- Hidden while Mesh Scale Mode is Uniform: setting it fails, leaving
    -- the visible mesh at its template scale.
...
```lua
ns:compile()    -- yields, then returns {success, errors, error_count,
                -- transaction_checkpoint=true}
ns:save()       -- returns true
```

`compile()` is an explicit transaction checkpoint. Niagara compilation can
reconstruct graph nodes and pins, so the binding first yields and closes the
current Lua edit transaction, then compiles on the next editor tick and resumes
with authoritative diagnostics. Treat a successful compile as committed editor
state: validate every requested input before calling it, stop on a failed result,
and call `save()` only after the compiled readback is acceptable.
...
### Inspecting placed runtime instances

`runtime_instances()` reads live state from placed `UNiagaraComponent`
instances that reference this system. It is asset-scoped and plural: without
filters it returns every matching component in placed editor, PIE, and game
preview worlds instead of guessing which instance you meant.

```lua
local ns = open_asset("/Game/VFX/NS_CustomCubeRing")
local runtime = ns:runtime_instances({
  world = "pie", -- "editor", "pie", "game_preview", or an exact world path
  actor_label = "CustomNiagaraActor", -- exact, case-sensitive
  component = "NiagaraComponent0",    -- exact UObject name
})
assert(runtime and runtime.ok and runtime.instance_count == 1)

local instance = runtime.instances[1]
print(instance.component_path)
print(instance.execution_state, instance.active, instance.complete)

if instance.emitter_snapshot_available then
  for _, emitter in ipairs(instance.emitters) do
    print(
      emitter.name,
      emitter.sim_target,
      emitter.execution_state,
      emitter.particle_count,
      emitter.count_accuracy
    )
  end
else
  print(instance.emitter_snapshot_status)
end
```

Use `component_path` from a previous result when you need a stable singular
identity:

```lua
local exact = ns:runtime_instances({
  component_path = runtime.instances[1].component_path,
})
```

Important semantics:

- Actor and component filters are exact. Duplicate actor labels across worlds
  return `status="ambiguous_target"` plus `candidates`; the binding never picks
  the first match.
- Safe component state remains available when the Niagara debugger snapshot is
  unavailable. Check `emitter_snapshot_available` before reading counts.
- Unavailable counts are omitted, not reported as zero. A real zero is returned
  only inside an available emitter snapshot.
- GPU counts use `count_accuracy="latent_gpu"` because Niagara's GPU readback
  can lag the current frame. CPU rows use `"snapshot_game_thread"`.
- The exported Niagara debugger snapshot flushes rendering and scans all
  Niagara components. Each call takes a fresh snapshot; use this for verification
  and diagnostics, not per-frame polling.
- Runtime inspection never forces solo mode, advances simulation, parses logs,
  or mutates the component.

## Discovery escape hatches

- `ns:help()` — every method on the system object
- `ns:info()` — emitter/module/renderer/user-param counts
- `ns:runtime_instances(opts?)` — placed component execution state and
  diagnostic per-emitter particle snapshots
